...
#                        CLASS_arg.json_data["common"]["targ_va"] = str(target_day * 0.37 * CLASS_arg.time_baseline)
        ...
        def generate_process(self, CLASS_arg):
            ...
            CLASS_arg.json_data.update({
                ...
                "common": {
                    ConfigKeys.GENERAL_INFO: str(self.line_edit_general_info.text()).strip() + f"_{today}_Mat+{return_value}",
                    ConfigKeys.BASE: str(CLASS_arg.time_baseline),
                    ConfigKeys.TARG_DA: str(float(CLASS_arg.target_day) * float(CLASS_arg.time_baseline)),
                    ConfigKeys.TARG_VA: str(float(CLASS_arg.target_day) * 0.37 * float(CLASS_arg.time_baseline)),
                    ConfigKeys.ENPREV: ""
                }
            })
            CLASS_arg.encrypt()
